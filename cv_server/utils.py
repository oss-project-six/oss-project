...
def draw_axis(img, yaw, pitch, roll, tdx=None, tdy=None, size=100, fov_deg=15, segments=30):
    ...
    for i in range(segments + 1):
        angle = -fov_rad / 2 + i * (fov_rad / segments)
        yaw_offset = yaw + angle
        x = size * (sin(yaw_offset)) + tdx
        y = size * (-cos(yaw_offset) * sin(pitch)) + tdy
        points.append((int(x), int(y)))

    center_point = (int(tdx), int(tdy))
    polygon_points = [center_point] + points

    # 반투명 처리를 위한 마스크 생성
    overlay = img.copy()
    cv2.fillPoly(overlay, [np.array(polygon_points, dtype=np.int32)], color=(200, 200, 0))
    
    # alpha blending
    alpha = 0.3  # 투명도 조절 (0: 완전 투명, 1: 불투명)
